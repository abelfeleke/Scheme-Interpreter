...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = rest.map(lambda operands : scheme_eval(operands,env))
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        """
        # BEGIN PROBLEM 10
        frameinstance = Frame(self)
        if formals.__len__() != vals.__len__():
            raise SchemeError()
        while formals != nil:
            frameinstance.define(formals.first,vals.first)
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        if self.env == None:
            return Frame().make_child_frame(self.formals, args)
        else:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.bindings[target.first] =LambdaProcedure(target.rest,expressions.rest, env)
        return target.first
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if expressions.rest == 'else':
                return eval_all(clause.rest, env)
            elif scheme_truep(clause.first):
                if clause.rest == nil:
                    return test
                else:
                    return eval_all(clause.rest, env)
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        evaluated_values, symbols = scheme_eval(bindings.first.rest.first, env), Pair(bindings.first.first,symbols)
        check_formals(symbols)
        values = Pair(evaluated_values, values)
